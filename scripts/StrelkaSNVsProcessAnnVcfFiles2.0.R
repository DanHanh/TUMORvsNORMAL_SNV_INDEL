...
AlelleFrequencyNormal <- mapply(function(f,n,ref,alt)
    round(as.numeric(unlist(strsplit(unlist(n)[match(paste0(alt, "U"), unlist(f))], split=","))[1]) / # alt allele count
      (as.numeric(unlist(strsplit(unlist(n)[match(paste0(alt, "U"), unlist(f))], split=","))[1])
       +  # alt + ref allele count
      as.numeric(unlist(strsplit(unlist(n)[match(paste0(ref, "U"), unlist(f))], split=","))[1])  
      ),5)
  , f,n,ref,alt)

AlelleFrequencyTumor <- mapply(function(f,t,ref,alt)
  round(as.numeric(unlist(strsplit(unlist(t)[match(paste0(alt, "U"), unlist(f))], split=","))[1]) / # alt allele count
    (as.numeric(unlist(strsplit(unlist(t)[match(paste0(alt, "U"), unlist(f))], split=","))[1])
     +  # alt + ref allele count
       as.numeric(unlist(strsplit(unlist(t)[match(paste0(ref, "U"), unlist(f))], split=","))[1])  
    ),5)
  , f,t,ref,alt)
...
#a <- unique(a$SYMBOL)
#a <- a[a != ""]
#write.table(a, "./strelka_PASS_H_geneList.txt", col.names = FALSE, row.names = FALSE, quote = FALSE)
